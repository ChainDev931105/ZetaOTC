...
const PriceStatus = ["Unknown", "Trading", "Halted", "Auction"];
const CorpAction = ["NoCorpAct"];
const PriceType = ["Unknown", "Price", "TWAP", "Volatility"];
...
export const createPriceFeed = async ({
  ...
  const conf = confidence == undefined ? initPrice / 100 : confidence; // default conf is 1/100 of init price
  await oracleProgram.rpc.initialize(
    new BN(initPrice * 10 ** -expo),
    expo,
    new BN(conf * 10 ** -expo),
    {
      accounts: { price: keypair.publicKey },
      signers: [keypair],
      instructions: [
...
export const ERR_INVALID_ARG_TYPE = (name, expected, actual) =>
  new Error(
...
export const ERR_OUT_OF_RANGE = (str, range, received) =>
  new Error(
...
export function boundsError(value, length) {
...
export function readBigInt64LE(buffer, offset = 0) {
...
export const parsePriceInfo = (data, exponent) => {
  // Aggregate price.
